...
fun compileProject(lookup: IRModuleLookup): Map<String, ByteArray> {
    return lookup.nativeModules.associate { module ->
        module.name.replace("::", ".") to compileModule(module, lookup)
    }
}

fun compileModule(module: IRModule, lookup: IRModuleLookup): ByteArray {
...
private fun boxOrIgnore(mv: MethodVisitor, type: Type) {
    when(type) {
...
private fun unbox(mv: MethodVisitor, type: Type)  {
    when (type) {
...
fun generateJVMFunctionSignature(argTypes: List<Type>, returnType: Type): String {
